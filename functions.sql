...
begin
    ...
    begin
        select max(warenkorbeintrag_id)+1 into next_id from system.warenkorbeintrag;
    end;
    ...
    begin
        select
            w.anzahl, w.warenkorbeintrag_id
        into
            article_count, waren_id
        from
            system.warenkorbeintrag w
        where
            treat(deref(w.kunde) as kunde_typ).kunden_id = kunden_id
        and
            treat(deref(w.artikel) as softwareartikel_typ).artikel_id = article_id;
    ...
    if article_count >= 1 then
        ...
        set
            w.anzahl = w.anzahl+1
        where
            w.warenkorbeintrag_id = waren_id;
...
begin
    begin
    select
        w.anzahl, w.warenkorbeintrag_id into article_count, waren_id
    from
        system.warenkorbeintrag w
    where
        treat(deref(w.kunde) as kunde_typ).kunden_id = kunden_id
